...
public class SPActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button btn1 = findViewById(R.id.save1);
        btn1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent ISP= new Intent();
                Bundle bundle = new Bundle();
                bundle.putCharSequence("name", buffer);
                bundle.putInt("total",total);

                ISP.putExtra("bundle",bundle);
                setResult(RESULT_OK,ISP);
                finish();
            }
        });
    ...
    private void initView() {
        ListView listview = (ListView) findViewById(R.id.splistview); // 获取列表视图
        // 创建一个list集合
        listview.setAdapter(new SPAdapter(this));
    }
